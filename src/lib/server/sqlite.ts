...
export function getInitialTracks(limit = 5, skip = 0, q = ''): Track[] {
	const sql = `
    select 
    (SELECT count(*) from Track WHERE UPPER(Name) LIKE UPPER('%' || $q || '%')) AS row_count,
    ...
    WHERE UPPER(trackName) LIKE UPPER('%' || $q || '%')
    LIMIT $limit OFFSET $skip
    `;
	const stmnt = db.prepare(sql);
	const rows = stmnt.all({ limit, skip, q });
